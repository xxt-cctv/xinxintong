    ...
    var requestGeoPosition = function () {
        ...
        if (nav !== null) {
            ...
            if (geoloc !== null) {
                ...
                window.kzrlGeo.onLocationFail && typeof window.kzrlGeo.onLocationFail === 'function' && window.kzrlGeo.onLocationFail("Geolocation API is not supported in your browser");
            }
        } else {
            window.kzrlGeo.onLocationFail && typeof window.kzrlGeo.onLocationFail === 'function' && window.kzrlGeo.onLocationFail("Navigator is not found");
        }
    };
    window.kzrlGeo = {};
    if (/Android/i.test(navigator.userAgent) || /iPhone/i.test(navigator.userAgent) || /iPad/i.test(navigator.userAgent)) {
        if (/CntvNews/.test(navigator.userAgent)) {
            window.setLocationYsxw = function (lng, lat) {
                if (lng == 0 || lat == 0)
                    window.kzrlGeo.onLocationFail && typeof window.kzrlGeo.onLocationFail === 'function' && window.kzrlGeo.onLocationFail("lat or lng is empty");
                else
                    window.kzrlGeo.onLocationReady && typeof window.kzrlGeo.onLocationReady === 'function' && window.kzrlGeo.onLocationReady(lat, lng);
            };
            if (window.position && window.position.setLocationInfo)
